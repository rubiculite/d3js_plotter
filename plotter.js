...
function plotter(element_id,data) {
   ...
   gPlotContainer
      ...
      .on("mouseover",function(){
         gPlotContainer.selectAll(".x-xhair, .y-xhair, .xhair-annotation").style("display",null);
      })
      .on("mouseout", function(){
         gPlotContainer.selectAll(".x-xhair, .y-xhair, .xhair-annotation").style("display","none");
      })
      .on("mousemove",function(){
         var coords = d3.mouse(this);
         var xPixels = coords[0];
         var yPixels = coords[1];
         var annotation = ordinats(xPixels,yPixels)
         gPlotContainer.select("line.x-xhair").attr("x1",xPixels).attr("x2",xPixels);
         gPlotContainer.select("line.y-xhair").attr("y1",yPixels).attr("y2",yPixels);
         gPlotContainer.select("text.xhair-annotation")
